...
public:
    ...
    std::array<result_type, 4> state; // (256 bit state)
    ...
    constexpr void jump (const decltype(state) &) noexcept;

    // state equivalent to (2^128) generator function calls:

    constexpr void jump () noexcept;

    // state equivalent to (2^192) generator function calls:

    constexpr void long_jump () noexcept;


    // Stafford's Mix13 variant for splitmix:
...
constexpr void
X256PP::jump (const decltype(state) & poly) noexcept
{
    decltype(state) jbuf {};

    for (auto j : poly) // apply GF(2) (jump) polynomial to state:
    {
